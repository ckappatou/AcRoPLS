...
class BBFunc:
    def __init__(self, Data, *, Rob=1, Dis=1, SNV=True, SG=True, OF=1, MMsource=0, random=0, stepWs=True, pltOn=False, pltName=False):
        self.rob_def = Rob
        self.dis = Dis
        self.of = OF
        ...
        scipy.io.savemat('cs2matlab.mat', dict(x=x_train1, y=self.y_train1, z=s_id_train1, x_t=x_test1, y_t=self.y_test1,
                                           z_t=s_id_test1, x_v=x_val1, y_v=self.y_val1,
                                           z_v=s_id_val1))
    ...
    def evaluate(self,x):
        ...
        M1sumA, M2sumA = mm.momMatch(pls_raman_calibration, x_test_snv_savgol, self.y_test,
                                CLASSID=self.id_test, cat_rob_index=2, subclass_index=1, \
                                     rob_def=self.rob_def, dis=self.dis)

        M1sumB, M2sumB = mm.momMatch(pls_raman_calibration, x_test_snv_savgol, self.y_test,
                                CLASSID=self.id_test, cat_rob_index=1, subclass_index=2, \
                                     rob_def=self.rob_def, dis=self.dis)
        ...
        MMA = M1sumA + beta * M2sumA # scale
        MMB = M1sumB + beta * M2sumB  # dos
...
# spectra plots
bb_func = BBFunc(DataFile, Rob=Robmetric, Dis=DisMeas, pltOn=True, pltName='CS2_spectraACC')
bb_func.evaluate([1, 0, 1, 7]) #acc
bb_func = BBFunc(DataFile, Rob=Robmetric, Dis=DisMeas, pltOn=True, OF=2, pltName='CS2_spectraSC')
bb_func.evaluate([4, 2, 3, 1]) #rob sc
bb_func = BBFunc(DataFile, Rob=Robmetric, Dis=DisMeas, pltOn=True, OF=2, pltName='CS2_spectraDOS')
bb_func.evaluate([7, 0, 1, 1]) #rob dos
bb_func = BBFunc(DataFile, Rob=Robmetric, Dis=DisMeas, pltOn=True, OF=2, pltName='CS2_spectraROB')
